...
print(f'Setting default publication date to today ({publication_date}) ...')
print(f'Trying to read explicit publication date from meta data at ({METADATATEX_PATH}) ...')
    ...
    for line in handle.readlines():
        if line.strip().startswith(PUB_DATE_GREP_TOKEN):
            ...
            if pub_date_read == 'PUBLICATIONDATE':
                print(f'- explicit publication date found as ({pub_date_read})')
                print(f'  + keeping default {publication_date}')
            else:
                publication_date = pub_date_read
                print(f'- found explicit publication date override as ({publication_date})')
            break
    else:
        print(
...
print(f'Validating date to be in format ({FORMAT_DATE}) that is DD MON YYYY')
try:
    ref = dti.datetime.strptime(publication_date, FORMAT_DATE).date().strftime(FORMAT_DATE).upper()
    if ref == publication_date:
        print(f'- publication date ({publication_date}) is valid')
    else:
        print(f'ERROR: full cycle conversion of found date ({publication_date}) is not the date itself ({ref})')
        sys.exit(1)
except Exception as err:
    print(f'ERROR: full cycle conversion of found date ({publication_date}) failed with: {err}')
    sys.exit(1)

print(f'Reading changes data from ({CHANGES_PATH}) ...')
with open(CHANGES_PATH, 'rt', encoding=ENCODING) as handle:
    changes = yaml.safe_load(handle)

print('Plausibility tests for changes ...')
for slot, change in enumerate(changes['changes'], start=1):
    if sorted(change) != sorted(COLUMNS_EXPECTED):
        ...
        print(f'- but found: ({change}) for entry #{slot}')
        sys.exit(1)
...
for change in changes['changes']:
    ...
    rows.append(ROW_TEMPLATE.replace('issue', issue).replace('date', date_rep).replace('summary', summary))

with open(PUBLISHER_TEMPLATE_PATH, 'rt', encoding=ENCODING) as handle:
    lines = [line.rstrip() for line in handle.readlines()]

print('Weaving in the changes ...')
