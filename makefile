...
pytest = pytest --cov=$(package) --cov-report term-missing:skip-covered --cov-branch --log-format="%(levelname)s %(message)s"
types = mypy $(package)
...
.PHONY: dlstats
dlstats:
	pypistats python_minor --json --monthly pyproject.toml > etc/monthly-downloads.json
